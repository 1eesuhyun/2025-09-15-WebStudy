     ...
     2. 지시자
        ...
        속성
         - contentType : 변환(브라우저에서 출력 형식)
           ...
           text/plain
         - import : 라이브러리 읽기
         - errorPage : 에러 시 이동하는 파일 지정
        include : JSP안에 다른 JSP를 읽어올 때 사용
          |
         속성
          - file => 고정 => <jsp:include>
                           ----
     ...
     3. 내장객체
        ...
        1) request 객체
           ...
           *** 메소드
           ...
           getParameter(String name) : 요청 데이터를 받는 경우
           ...
           getRequestURL() : 사이트 전체 주소
                             ============
                           : ContextPath
           getContexttPath() : /JSPBasicPrject_4
           getServerInfo() : 서버 IP
           getProtocol() : http
           getPort() : 80
           
           클라이언트 정보
           getRemoteAddr() : 크라이언트의 IP => log
           => 조회수
           setCharacterEncodeing("UTF-8") => Decoding
           --------------------------------
        2) response 객체
           - 응답 정보
             setContentType()
