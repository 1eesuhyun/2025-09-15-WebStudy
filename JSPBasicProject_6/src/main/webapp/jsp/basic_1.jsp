...
<%--
   ...
   = response : 브라우저로 전송 할 응답 정보 / 화면 이동
   = pageContext : 웹 흐름 제어
                   include / forward
   ...
   = out
     ...
     ========
     out.println() = <%= %>
     out.getBufferSize() : 총 버퍼 크기 => 8kb
     out.getRamaining() : 남아있는 버퍼
     출력 : HTML이 출력을 다 하지 않는다
           buffer를 증가
           <%@ page buffer="16kb"%>
     => 버퍼를 지운다 : flush() => auto
   ...
   -------------------- 기본
    HTTP의 기본 satteless : 무상태
         ...
         저장 => 클라이언트(브라우저) : cookie
          ...
          ----------------------------------
     저장 위치     클라이언트        서버
     저장 형태     문자열           서버메모리(객체단위)
     보안성        낮음            높음
     지속시간      브라우저 종료후에도 유지
                 설정이 가능 setMaxAge(60*60*24)
     속도         빠름(클라이언트)    다소 늦음(서버 접근 필요)
     주 용도       사용자 편의       사용자 인증 / 상태 관리
     
     ----------------------------------------------------
     1. HTTP 기본(요청 == 응답 => 연결 종료)
        | 필요한 데이터를 저장할 수 있는 기억장치가 필요
        | cookie / session(접속한 서버에 저장)
            |
        본인 컴퓨터에 저장
